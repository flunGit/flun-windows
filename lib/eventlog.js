 ...
 *     log.error('发生了错误!!!');
 *
 * 一些较少使用的选项也可以通过日志记录获得;例如,审计成功和失败消息：
 ...
 *     log.info('发生了不同的事情！', 1002, ()=>{
 *       console.log('发生了不同的事情！');
 *     });
 *
 * 默认情况下,事件日志都属于`APPLICATION`范围;但是,也可以使用`SYSTEM`日志;
 ...
 *     });
 */
const { exec } = require("child_process"), { promisify } = require("util"), wincmd = require('./binaries'),
  execAsync = promisify(exec), eventLogs = ['APPLICATION', 'SYSTEM'],
...
class EventLogger {
  ...
  #initializeProperties(config) {
    ...
    this.source = source;     // 日志来源(默认为Node.js)
    this.eventLog = eventLog; // 事件日志名称(默认为APPLICATION)
  }
  ...
  async #write(log = 'APPLICATION', src = '未知应用程序', type = 'INFORMATION', msg, id = 1000, callback) {
    ...
    const pMsg = msg.replace(/\r\n|\n\r|\r|\n/g, "\f"),  // 替换换行符
      ...
      vId = typeof id === 'number' && id > 0 ? id : 1000,
      vSrc = src.trim(),
      command = `eventcreate /L ${vLog} /T ${vType} /SO "${vSrc}" /D "${pMsg}" /ID ${vId}`;

    try {
      await execAsync(command), callback?.();
    } catch (error) {
      if (error?.message?.includes("Access is Denied")) await this.#elevateCommand(command, callback);
      else {
        callback?.(error);
        throw error;
      }
