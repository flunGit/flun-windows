 ...
 */
const { exec, execSync } = require('child_process'), { promisify } = require('util'), path = require('path'), fs = require('fs'),
  { EventEmitter } = require('events'), wincmd = require('./binaries'), winsw = require('./winsw'), Logger = require('./eventlog'),

  writeFileAsync = promisify(fs.writeFile), mkdirAsync = promisify(fs.mkdir),

  daemonDir = 'daemon', PermError = '权限被拒绝,需要管理员权限;请以管理员身份重新运行此脚本;',
  wrapper = path.resolve(path.join(__dirname, './wrapper.js')), nameRegex = /[<>:"\\/|?*]/g;
...
class Service extends EventEmitter {
  ...
  #initializeProperties(config) {
     ...
     * 如果需要指定特定用户或凭据来管理服务,需设置属性:`domain`、`account`和`password`;
     * 默认情况下,域设置为本地计算机名,但可以使用Active Directory或LDAP域覆盖;
     */
    this.user = { account: undefined, password: null, domain, ...user };
   ...
   */
  async start() {
    if (!this.exists) throw new Error(`服务"${this.name}"不存在;`);
    ...
    try {
      ...
      if (error.message.includes('already been started')) this.log.warn('尝试启动服务失败,因为服务已经在运行;');
      else this.#checkPermError(error);
      this.emit('error', error);
    }
  }
  ...
  async stop() {
    try {
      ...
      if (error.code === 2) this.log.warn('服务未运行或已停止;'), this.emit('alreadystopped');
      else this.#checkPermError(error), this.emit('error', error);
    }
  }
  ...
  async #execute(cmd, options = {}) {
    const execOptions = { ...options, shell: true, windowsHide: true };
    return new Promise((resolve, reject) => {
      const callback = (error, stdout, stderr) => {
        if (error) this.#checkPermError(error), reject(error);
        else resolve({ stdout, stderr });
      };

      // 检查用户凭据
      if (this.user.account && this.user.password) {
        const runasCmd = `runas /profile /user:"${this.user.domain}\\${this.user.account}" "${cmd}"`;
        return exec(runasCmd, execOptions, callback);
      }
      // 检查sudo密码
      const executor = this.sudo?.password ? (cmd, opts, cb) => wincmd.sudo(cmd, this.sudo.password, opts, cb) : wincmd.elevate;
      executor(cmd, execOptions, callback); // 默认使用提升权限
    });
  }

  // 检查权限错误
  #checkPermError(error) {
    const errorRegex = /Administrator access|Access is denied|AccessDenied/i;
    if (errorRegex.test(error.message)) throw new Error(PermError);
