 ...
 */
const { path, fs, net, fork } = require('./shared'), yargs = require('yargs/yargs'), { hideBin } = require('yargs/helpers'),
  Logger = require('./eventlog'),

  // 时间窗口（秒）,默认时间窗口内最大重启次数,默认重启等待时间（秒）,默认等待时间增长系数
  timeWindow = 60, defaultMaxRestarts = 3, defaultWaitTime = 1, defaultGrowth = 0.25, booleanChoices = ['y', 'n', 'yes', 'no', 'true', 'false'],
