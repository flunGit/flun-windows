const { path, fs, exec } = require('./shared'), os = require('os'), vbsPath = path.resolve(__dirname, '../bin/elevate.vbs'),
    ...
    elevate = (cmd, options, callback) => {
        const p = _params(options, callback), tmpDir = os.tmpdir(),
            ...
            // 使用固定文件名
            [outputFile, exitFile, vbsFile] = ['flun_el_output.txt', 'flun_el_exit.txt', 'flun_el.vbs']
                .map(name => path.join(tmpDir, name)), escapedCmd = cmd.replace(/"/g, '""').replace(/%/g, "%%"),
            vbsScript = readFileSafe(vbsPath).replace(/{command}/g, escapedCmd); // 生成VBScript文件内容
        ...
        exec(`wscript.exe "${vbsFile}"`, { timeout: 20000 }, () => {
            setTimeout(() => {
                const output = readFileSafe(outputFile).replace(/\r\n$|\n$/, ''), exitCodeStr = readFileSafe(exitFile).trim(),
                    exitCode = exitCodeStr ? parseInt(exitCodeStr, 10) : 1;

                // 清理临时文件
                [outputFile, exitFile, vbsFile].forEach(f => {
                    try { fs.unlinkSync(f) } catch (e) { }
                });

                if (exitCode !== 0) {
                    const err = new Error(`命令执行失败,退出码:${exitCode}`);
                    err.code = exitCode, err.output = output; p.callback(err, output, '');
                }
                else p.callback(null, output, '');
            }, 500);
        });
    },
